...
function generate() {
  ...
  while (particles.length < COUNT) {
    ...
    if (Math.random() > Math.min(1, Math.pow(center / 1, 0.85))) continue;
...
function spawnPhoto() {
  ...
  photoParticles.push({
    img,
    x: (Math.random() < 0.5 ? -1 : 1) * (1.8 + Math.random() * 0.6),
...
function draw(now) {
  ...
  /* PHOTOS */
  if (!building && photoParticles.length < 8 && Math.random() < 0.01) {
    spawnPhoto();
  }
  ...
  for (let i = photoParticles.length - 1; i >= 0; i--) {
    ...
    ctx.drawImage(
      p.img,
      p.x * w * s * 0.25 - p.img.width * s * 0.15,
      p.y * h * s * 0.25 - p.img.height * s * 0.15,
      p.img.width * s * 0.3,
      p.img.height * s * 0.3
    );
  }
